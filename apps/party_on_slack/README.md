...
- [Party on Slack 機能一覧](#party-on-slack-機能一覧)
  - [目次](#目次)
  - [1. 初期セットアップとアクセス制御](#1-初期セットアップとアクセス制御)
    - [1.1 アプリのインストール](#11-アプリのインストール)
    - [1.2 グローバルロック設定機能](#12-グローバルロック設定機能)
    - [1.3 ワークスペース許可設定](#13-ワークスペース許可設定)
  - [2. APIキー設定と基本認証](#2-apiキー設定と基本認証)
    - [2.1 各種APIキーの登録・検証](#21-各種apiキーの登録検証)
      - [API key 登録・検証](#api-key-登録検証)
    - [2.2 使用モデル選択機能](#22-使用モデル選択機能)
    - [2.3 ワークスペースモデル設定](#23-ワークスペースモデル設定)
  - [3. 基本的なAIチャット機能](#3-基本的なaiチャット機能)
    - [3.1 チャンネルでのメンション応答](#31-チャンネルでのメンション応答)
    - [3.2 DMでの会話](#32-dmでの会話)
    - [3.3 スレッド内での継続的な会話](#33-スレッド内での継続的な会話)
    - [3.4 スレッドモデル変更機能](#34-スレッドモデル変更機能)
    - [3.5 チャンネルモデル変更機能](#35-チャンネルモデル変更機能)
    - [3.6 キャラクター設定（システムプロンプト）](#36-キャラクター設定システムプロンプト)
  - [4. ファイル処理機能](#4-ファイル処理機能)
    - [4.1 ファイル要約（Word、PPT、Excel、CSV）](#41-ファイル要約wordpptexcelcsv)
    - [4.2 URL要約](#42-url要約)
    - [4.3 画像認識（マルチモーダル）](#43-画像認識マルチモーダル)
    - [4.4 音声ファイルの文字起こし](#44-音声ファイルの文字起こし)
  - [5. コンテンツ生成機能](#5-コンテンツ生成機能)
    - [5.1 画像生成](#51-画像生成)
    - [5.2 音声生成](#52-音声生成)
    - [5.3 Canvas作成機能](#53-canvas作成機能)
  - [6. 検索・統合機能](#6-検索統合機能)
    - [6.1 Web検索機能](#61-web検索機能)
    - [6.2 Slackにきく機能（内部検索）](#62-slackにきく機能内部検索)
    - [6.3 Slackにきく機能（要約機能）](#63-slackにきく機能要約機能)
    - [6.4 Pinecone連携（ベクトルDB）](#64-pinecone連携ベクトルdb)
    - [6.5 Google スプレッドシートRAG](#65-google-スプレッドシートrag)
  - [7. 翻訳機能](#7-翻訳機能)
    - [7.1 スタンプ翻訳設定](#71-スタンプ翻訳設定)
    - [7.2 翻訳モデル選択](#72-翻訳モデル選択)
    - [7.3 翻訳先設定（スレッド/チャンネル）](#73-翻訳先設定スレッドチャンネル)
  - [8. 自動化・定期実行機能](#8-自動化定期実行機能)
    - [8.1 RSSコンシェルジュ](#81-rssコンシェルジュ)
    - [8.2 LLMログ転送](#82-llmログ転送)
  - [9. 外部サービス連携](#9-外部サービス連携)
    - [9.1 Google連携（カレンダー調整）](#91-google連携カレンダー調整)
